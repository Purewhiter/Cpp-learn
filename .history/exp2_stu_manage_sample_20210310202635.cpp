...
int add_file()
{
    fp.open("stu_data.txt", ios::out|ios::app);
    if (!fp)
    {
        printf("error\n");
        return -1;
    }
    fp << stArray[aCount-1].num << " " << stArray[aCount-1].name << " " << stArray[aCount-1].sex << " " << stArray[aCount-1].age << " " << stArray[aCount-1].addr << endl;
...
int display_file(void)
{
    st stu;
    fstream f;
    f.open("stu_data.txt", ios::in);
    // if()
    // {
    //     printf("文件打开失败！\n");
    //     exit(-1);
    // }
    f >> stu.num >> stu.name >> stu.sex >> stu.age >> stu.addr;
    while (!f.eof())
    {
        // if(fread(&stu, sizeof(stu),1,fp))
