 ...
 * @LastEditTime: 2021-06-20 17:17:23
 * @Description : 
 */
#include <Algorithm>
#include <chrono>
...
void BackTrack(int i)
{
    ...
    // double loss = tempProfit + (Capacity - tempWeight) * ((double)ItemSet[i].profit / ItemSet[i].weight);
    // for (int j = i; j < N; j++)
    // {
    ...
    if (tempWeight + ItemSet[i].weight <= Capacity) //进入该节点的右孩子（值为1的孩子）
    {
        ...
        BackTrack(i + 1);
        tempWeight -= ItemSet[i].weight; //回溯
        tempProfit -= ItemSet[i].profit;
    }
...
int main()
{
    ...
    cout << "The run time is "
         << double(duration.count()) / 1000 // * microseconds::period::num / microseconds::period::den
         << "ms" << endl;
    for (int i = 0; i < N; i++)
    {
        if (bestChoice[i] == 1)
